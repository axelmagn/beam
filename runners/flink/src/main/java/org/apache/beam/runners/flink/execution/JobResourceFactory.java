...
public class JobResourceFactory {
  ...
  ContainerManager containerManager(ArtifactSource artifactSource, ProvisionInfo jobInfo)
      throws IOException {
  ...
  /** Create a new container manager from services */
  private ContainerManager containerManager(
      GrpcFnServer<SdkHarnessClientControlService> controlService,
      GrpcFnServer<GrpcLoggingService> loggingService,
      GrpcFnServer<ArtifactRetrievalService> artifactRetrivalService,
      GrpcFnServer<StaticGrpcProvisionService> provisionService) {
    return SingletonDockerContainerManager
        .forServers(controlService, loggingService, artifactRetrivalService, provisionService);
  }
